...
1) Создана база данных (на основе SQLite), хранящая информацию об имени пользователя, его email и общее количество набранных очков. Реализована она в классе DataBaseUsers на основе класса SQLiteOpenHelper.
...
4) Появилась новая активити - турнирная таблица(реализованная в активити TableUsers), где выведены имя игрока и набранное количество очков. Таблица строится по данным из базы данных, с использованием ListView Adapter. Таблицу можно увидеть, кликнув по кнопке "Игроки" в главном меню.
5) Немного изменена структура главного меню: добавлен spinner с городами, при нажатии на который пользователю отображаются 4 города(текущий город указан в шапке спиннера). Кликая по новому городу пользователь меняет город, в котором будет играть при нажатии кнопки "Играть". 
6) Немного изменена структура функционирования главной карты. Теперь с помощью вспомогательного класса AppControler карта выстраивается по тем координатам, которые соответствуют выбранному городу. Это происходит с помощью функций getLatLng()(вызывается в MapsActivity при создании карты) и setLatLng()(вызывается в SignIn для сохранения координат выбранного города).
7) Изменена структура викторины: теперь, когда на главной карте выбирается полигон, то он сохраняется в AppConroler, затем активити NE вытаскивает нужную информацию, передавая ее, создавая newInstance - фрагмент карты(где пользователь будет выбирать ответ) и newInstance - фрагмент карты с ответом. Карты строятся теперь не по константным значениям, а по переданным от NE переменным.
8) Теперь поиск мест(достопримечательностей) происходит по переданному значению центра города, радиус поиска увеличен, чтобы мест было больше.
9) Создана простая система уровней: при каждом 20 верном ответе уровень игрока увеличивается и ему дается "фишка" в качестве награды.
10) Для хранения фишек создана база данных DataBaseAwards, в которой лежат ссылки на картинки соответствующие фишкам. 
11) В главном меню добавлена кнопка "Мои фишки", которая открывает активити ShowAwards, которая отображает все фишки игрока. Для отображения картинок использованы методы Picasso, уже использовавшиеся ранее.
12) Для корректности доступа к данным в AppControler реализована дополнительная проверка не только на подключение GoogleGlient, но и проверка на "существование" текущего пользователя. Если оба условия выполнены, то отображены кнопки "Выйти" и "Мои фишки" на главном меню, иначе они скрыты.

Заключение 
------------------------------------------------------------------------------------------------------------------------------

В процессе работы были введены некоторые изменения в изначальный план действий, так как они упрощали работу и были более оптимальными(например, не создание базы данных фотографий мест, а выкачку фотографий по запросу). Данные изменения коснулись только методов выполнения, но не самих задач.

В результате мы имеем работающее Android приложение, функционал которого в полной мере поддерживается 25 api, но оно также работает и на более старых версиях. 

Приложение является обучающей игрой, задача которого - помочь пользователю изучить достопримечательности города. В итоговой версии к изучению доступны города: Москва, Дублин, Цюрих, Ташкент.

Пользователь может активно взаимодействовать с картой, получая верный ответ, если его ответ на вопрос был неверен (весь процесс описан в разделах выше). Сам процесс игры довольно-таки приятен, так как есть не только вопрос в текстовом виде, но и фотография места, которая помогает вспомнить месторасположение места и развивает визуальную память.

Пользователи остаются зарегистрированными в системе, так как использована регистрация через Google аккаунт. С помощью регистрации мы получаем данные о пользователе, сохраняем их в базе данных, в которой также хранится другая необходимая информация об игроках.

Реализована соревновательная возможность, которая также помогает в игровом процессе. Пользователь имеет доступ к турнирной таблице, видит свои очки и уровень, а также данные о других пользователях. Добавляет азарта возможность получить фишку города, чтобы иметь ее в своей колекции фишек.

Все поставленные задачи были выполнены в полном объеме, проект завершен.

Возможности дальнейшего совершенствования:
1) Дизайн приложения может быть усовершенствован.
2) Может быть увеличено количество городов доступных для изучения.
3) Фишки могуть стать не фоторграфиями, а уникальными анимационными объектами, доступными только в этой игре.
4) Интеграция с GooglePlayGames и GoogleStreetView.

Возможность использования:
Игра доступна для всех пользователей, не имеет возрастных ограничений и преквизитов. 

В остальном, вся техническая и функциональная часть реализована.
